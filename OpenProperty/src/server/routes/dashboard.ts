...
export function mountDashboardRoutes(app: Hono<AppEnv>) {
  app.get("/api/dashboard/summary", async (c) => {
    ...
    const [
      ...
      safeGet<{ n: number }>(
        "SELECT COUNT(*)::int as n FROM properties WHERE organization_id = $1",
        [o],
        { n: 0 },
      ),
      safeGet<{ n: number }>(
        "SELECT COUNT(*)::int as n FROM units WHERE organization_id = $1",
        [o],
        { n: 0 },
      ),
      safeGet<{ n: number }>(
        "SELECT COUNT(*)::int as n FROM units WHERE organization_id = $1 AND status = 'occupied'",
        [o],
        { n: 0 },
      ),
      safeGet<{ n: number }>(
        "SELECT COUNT(*)::int as n FROM units WHERE organization_id = $1 AND status = 'vacant'",
        [o],
        { n: 0 },
      ),
      safeGet<{ n: number }>(
        "SELECT COUNT(*)::int as n FROM leases WHERE organization_id = $1 AND status = 'active'",
        [o],
        { n: 0 },
      ),
      safeGet<{ n: number }>(
