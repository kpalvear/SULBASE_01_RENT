...
app.get("/api/vendors", async (c) => {
  const o = orgId(c);
  const rows = await query(c, "SELECT * FROM vendors WHERE organization_id = $1 ORDER BY name", [o]);
...
app.post("/api/vendors", async (c) => {
  ...
  const row = await get(
    ...
    [o, d.name, d.category ?? "general", d.phone ?? null, d.email ?? null, d.notes ?? null, d.color ?? "slate"],
  );
...
app.put("/api/vendors/:id", async (c) => {
  ...
  const row = await get(c, "SELECT * FROM vendors WHERE id = $1 AND organization_id = $2", [id, o]);
