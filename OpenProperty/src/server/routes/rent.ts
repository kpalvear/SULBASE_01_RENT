...
app.get("/api/rent-charges", async (c) => {
  ...
  const rows = await query(c, `${chargeSelect(extra)} ORDER BY c.due_date, p.name, u.name`, params).catch(() => []);
...
app.post("/api/rent-charges", async (c) => {
  ...
  if (!inserted) {
    const existing = await get(c, `${chargeSelect("AND c.lease_id = $2 AND c.period = $3")}`, [o, d.lease_id, d.period]);
...
app.delete("/api/rent-charges/:id", async (c) => {
  ...
  if (!id) return c.json({ error: "Invalid ID" }, 400);
  const o = orgId(c);
  const r = await run(c, "DELETE FROM rent_charges WHERE id = $1 AND organization_id = $2", [id, o]);
  if (!r.changes) return c.json({ error: "Not found" }, 404);
...
app.post("/api/payments", async (c) => {
  ...
  await run(
    ...
    [o, d.charge_id, d.paid_at ?? null, String(d.amount), d.method ?? "cash", d.reference ?? null, d.notes ?? null],
  );
