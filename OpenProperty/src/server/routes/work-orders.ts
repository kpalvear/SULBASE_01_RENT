...
app.put("/api/work-orders/:id", async (c) => {
  ...
  if (data.cost !== undefined && data.cost !== null) data.cost = String(data.cost) as unknown as number;
...
app.delete("/api/work-orders/:id", async (c) => {
  ...
  if (!id) return c.json({ error: "Invalid ID" }, 400);
  const o = orgId(c);
  const r = await run(c, "DELETE FROM work_orders WHERE id = $1 AND organization_id = $2", [id, o]);
  if (!r.changes) return c.json({ error: "Not found" }, 404);
